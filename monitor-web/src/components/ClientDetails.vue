...
const enableNodeEdit = () => {
  ...
  nodeEditor.name = details.base.node
  nodeEditor.location = details.base.location
}

const submitNodeEdit = () => {
  post('/api/monitor/node', {
    id: props.id,
    node: nodeEditor.name,
    location: nodeEditor.location
  }, () => {
        ...
        <div v-else>
          ...
          <div style="display: inline-block;height: 15px">
            <div style="display: flex">
              <el-select v-model="nodeEditor.location" style="width: 80px" size="small">
                <el-option v-for="item in locations" :value="item.name">
                  <span :class="`flag-icon flag-icon-${item.name}`"></span>&nbsp;
                  {{item.desc}}
                </el-option>
              </el-select>
              <el-input v-model="nodeEditor.name" style="margin-left: 10px"
